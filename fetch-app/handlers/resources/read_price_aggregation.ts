...
// ---------------------------------------------- route handler
const handleReadPriceAggregation = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const aggregationResults: IAggregationResult[] = sorted
      .reduce((results, resource): IAggregationResultWithValues[] => {
        const { area_provinsi, price, tgl_parsed } = resource;

        if (!area_provinsi || !tgl_parsed || !price) return results;
              ...
              aggregates: [
                {
                  ...
                  max: parseInt(price),
                  avg: parseInt(price),
                  median: parseInt(price),
                  values: [parseInt(price)],
                  resources: [resource],
                },
              ],
            },
                ...
                return {
                  ...
                  aggregates: result.aggregates.map((a) => {
                    if (a.week === foundByWeek.week) {
                      return {
                        ...a,
                        min: a.min > parseInt(price) ? parseInt(price) : a.min,
                        max: a.min > parseInt(price) ? a.min : parseInt(price),
                        avg: average([...a.values, parseInt(price)]),
                        median: median([...a.values, parseInt(price)]),
                        values: [...a.values, parseInt(price)],
...
export default handleReadPriceAggregation;
